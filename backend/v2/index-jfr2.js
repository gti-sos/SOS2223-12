...
function loadBackend_jfr2(app) {
  ...
  app.get(BASE_API_URL + "/pollutions", (request, response) => {
    db.find({}, { _id: 0 }, (err, filteredList) => {
      //El id:0 = no incluir el campo _id
      ...
      if (err) {
        ...
        let datitos = filteredList
          .filter((x) => {
            return (
              (request.query.year == undefined ||
                parseInt(request.query.year) === x.year) &&
              (request.query.from == undefined ||
                parseInt(request.query.from) <= x.year) &&
              (request.query.to == undefined ||
                parseInt(request.query.to) >= x.year) &&
              (request.query.province == undefined ||
                request.query.province === x.province) &&
              (request.query.NO2_over == undefined ||
                parseFloat(request.query.NO2_over) <= x.NO2) &&
              (request.query.NO2_under == undefined ||
                parseFloat(request.query.NO2_under) >= x.NO2) &&
              (request.query.O3_over == undefined ||
                parseFloat(request.query.O3_over) <= x.O3) &&
              (request.query.O3_under == undefined ||
                parseFloat(request.query.O3_under) >= x.O3) &&
              (request.query.SO2_over == undefined ||
                parseFloat(request.query.SO2_over) <= x.SO2) &&
              (request.query.SO2_under == undefined ||
                parseFloat(request.query.SO2_under) >= x.SO2)
            );
          })
          .filter((x) => {
            // La paginación
  ...
  function pagination(request, lista) {
    ...
    if (limit < 1 || offset < 0 || offset > lista.length) {
      ...
      //slice es para coger una parte de la lista
  ...
  // Manejo de la ruta GET "/pollutions/:province"
  app.get(BASE_API_URL + "/pollutions/:province", (request, response) => {
    // Obtener el parámetro :province de la solicitud
    const province = request.params.province;
    // Obtener los parámetros de consulta from y to de la solicitud
    const from = request.query.from;
    const to = request.query.to;

    // Realizar una consulta a la base de datos
    db.find({}, (err, pollution) => {
      //parámetros from y to están presentes y no hay errores
      if (from && to && !err) {
        // from es mayor que to (rango de años inválido)
        if (from > to) {
          // Devolver una respuesta de estado 400 (Solicitud incorrecta) con un mensaje de error
          response
            .status(400)
            .json("El rango de años especificado es inválido");
        } else {
          // Filtrar los datos de la contaminación según la provincia, el rango de años y eliminar el campo _id
          const datosFiltrados = pollution.filter(
            (x) => x.province === province && x.year >= from && x.year <= to
          );
          // Devolver una respuesta de estado 200 (Éxito) con los datos filtrados
          ...
          // Imprimir en la consola la ruta y los parámetros de la solicitud GET realizada
          console.log(`/GET en /pollutions/${province}?from=${from}&to=${to}`);
        }
      } else if (!err) {
        // Filtrar los datos de la contaminación según la provincia y eliminar el campo _id
        const datosFiltrados = pollution.filter((x) => x.province == province);

        // Comprobar si no se encontraron datos filtrados (ruta no existente)
        if (datosFiltrados.length == 0) {
          // Devolver una respuesta de estado 404 (No encontrado) con un mensaje de error
        ...
        // Si hay un error en la base de datos, devolver estado 500 (Error interno del servidor)
        response.sendStatus(500);
        console.log("No se ha podido hacer la búsqueda");
      }
    });
  });
  ...
  app.get(BASE_API_URL + "/pollutions/:province/:year", (request, response) => {
    ...
    db.find({}, (err, pollution) => {
      if (!err) {
        var filtro = pollution.filter(
          (x) => x.province == province && x.year == year
        );
        if (filtro.length == 0) {
          response.status(404).json("La ruta solicitada no existe");
    ...
    if (province === body.province) {
      ...
      for (const field of requiredFields) {
        if (!request.body.hasOwnProperty(field)) {
          return response
            .status(400)
            .json(`Falta alguno de los campos: ${field}`);
        }
      }
      ...
      db.update(
        { province: province },
        {
          $set: { year: body.year, NO2: body.NO2, O3: body.O3, SO2: body.SO2 },
        },
        {},
        (err, numAffected) => {
          if (err) {
            ...
            console.log(
              "No se ha encontrado el objeto con la provincia especificada"
            );
            response
              .status(400)
              .send(
      ...
      console.log(
        "La provincia en la URL no coincide con la provincia en la solicitud"
      );
      response
        .status(400)
        .send(
  ...
  app.put(BASE_API_URL + "/pollutions/:province", (request, response) => {
    ...
    // Comprobar si la provincia en la URL coincide con la provincia en el cuerpo de la solicitud
    if (province === body.province) {
      const requiredFields = ["province", "year", "NO2", "O3", "SO2"];
      ...
      // Actualizar el objeto en la base de datos con los nuevos valores proporcionados
      db.update(
        { province: province },
        {
          $set: { year: body.year, NO2: body.NO2, O3: body.O3, SO2: body.SO2 },
        ...
        (err, numAffected) => {
          if (err) {
            // Si hay un error al actualizar el objeto, imprimir en la consola y devolver una respuesta de estado 500 (Error interno del servidor)
            console.log("Error actualizando el objeto: ", err);
            response.status(500).send("Error actualizando el objeto");
          } else if (numAffected === 0) {
            // Si no se encuentra el objeto con la provincia especificada, imprimir en la consola y devolver una respuesta de estado 400 (Solicitud incorrecta)
            console.log(
              "No se ha encontrado el objeto con la provincia especificada"
            );
            response
            ...
            // Si se actualiza correctamente el objeto en la base de datos, imprimir en la consola y devolver una respuesta de estado 200 (Éxito)
            console.log("Nuevo objeto actualizado en la base de datos");
            response.status(200).send("Actualizado");
          }
        }
      ...
      // Si la provincia en la URL no coincide con la provincia en la solicitud, imprimir en la consola y devolver una respuesta de estado 400 (Solicitud incorrecta)
      console.log(
        "La provincia en la URL no coincide con la provincia en la solicitud"
      );
      response
  ...
  app.delete(
    BASE_API_URL + "/pollutions/:province/:year",
    (request, response) => {
      const province = request.params.province;
      const year = request.params.year;
      db.remove(
        { province: province, year: parseInt(year) },
        {},
        (err, numRemoved) => {
          if (err) {
            console.log("Error para borrar todos los datos");
